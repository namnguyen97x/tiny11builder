...
Get-WindowsImage -ImagePath "$mainOSDrive\nano11\sources\install.wim"
$ImagesIndex = (Get-WindowsImage -ImagePath "$mainOSDrive\nano11\sources\install.wim").ImageIndex

if ($NonInteractive) {
    # Auto-detect edition based on VersionSelector
    Write-Output "Auto-detecting edition based on VersionSelector: $VersionSelector"
    $wimInfo = Get-WindowsImage -ImagePath "$mainOSDrive\nano11\sources\install.wim"
...
Copy-Item -Path "$PSScriptRoot\autounattend-nano11.xml" -Destination "$sysprepDir\autounattend.xml" -Force | Out-Null
...
Write-Output "Exporting final image to highly compressed ESD format..."
...
Remove-Item "$mainOSDrive\nano11\sources\install.wim" -ErrorAction SilentlyContinue
